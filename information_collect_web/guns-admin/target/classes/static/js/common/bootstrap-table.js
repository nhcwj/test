/**
 * @author zhirongrui <wenzhixin2010@gmail.com>
 * add by 2018/6/21
 * version: 1.12.1
...
(function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showColumns) {
            ...
            var keepOpenInput = this.$toolbar.find('.keep-open input');
    ...
    BootstrapTable.prototype.toggleColumn = function (index, checked, needUpdate) {
        var that=this;
        var tempItems=this.$toolbar.find('.keep-open input').prop('disabled', false);
        if (index === -1) {
            return;
        }
        if(index == -2){
            if(checked){
                tempItems.filter(':not(:checked)').prop('checked', true);
                tempItems.each(function (i,v) {
                        that.columns[i].visible = true;
                })
            }else{
                tempItems.filter(':checked').prop('checked', false);
                tempItems.each(function (i,v) {
                        that.columns[i].visible = false;
                })
                // tempItems.each(function (i,v) {
                //     if(i==1 || i==2){
                //         $(v).prop("checked",true);
